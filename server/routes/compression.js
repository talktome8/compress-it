...
const fileFilter = (req, file, cb) => {
    ...
    if (allowedTypes.includes(file.mimetype)) {
        ...
        cb(new Error(`Invalid file type: ${file.mimetype}. Only JPG, PNG, GIF, and WebP are allowed.`), false);
    }
};
...
router.post('/compress', async (req, res) => {
    try {
        ...
        const results = await Promise.all(
            files.map(file => compressionService.compressImage(file, compressionSettings))
        );
...
router.post('/download-zip', async (req, res) => {
    try {
        ...
        const zipPath = path.join(__dirname, '../../compressed', zipFilename);
        ...
        for (const file of files) {
            const filePath = path.join(__dirname, '../../compressed', file.compressedFilename);
            try {
                await fs.access(filePath);
                archive.file(filePath, { name: file.downloadName || file.compressedFilename });
            } catch (e) {
